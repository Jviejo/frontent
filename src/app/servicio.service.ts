...
export class ServicioService {
  ...
  async getDatosSerie() {
    ...
    try {
      ...
      if (datos1 === 1) { throw Error('no se puede'); }
    } catch (error) {
  ...
  getDatos() {
    ...
    const arrLLamadas = [
      this._http.get('http://192.168.99.1:8081/src/assets/datos.json', { headers }),
      this._http.get('http://192.168.99.1:8081/src/assets/datos.1.json', { headers })
    ];
