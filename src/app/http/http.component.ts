...
export class HttpComponent implements OnInit {
  ...
  loadData(filename, tipo) {
    this._http.get(`${this.apiUrl}/loadData/${filename}`, { responseType: tipo }).subscribe((datos: any) => {
      console.log(datos);
      if (tipo == 'blob') {
        var reader = new FileReader();
        reader.readAsDataURL(datos);
        reader.onloadend = () => {
          this.base64data = this._sanitize.bypassSecurityTrustUrl(reader.result);
        }
      }
  ...
  uploadDataMultiple($event) {
    ...
    this._http.post(`${this.apiUrl}/uploadMultiple`, formData).subscribe((i: any) => {
      this.files = i;
      console.log(i);
      const llamadas = this.files.file.map(i => this._http.get(`${this.apiUrl}/loadData/${i.filename}`, { responseType: 'blob' }))
      forkJoin(llamadas).subscribe(async imagenes => {
        ...
        imagenes.forEach(async (imagen: any) => {
          ...
          reader.onloadend = () => {
            this.imagenes.push(this._sanitize.bypassSecurityTrustUrl(reader.result))
          }
        })
